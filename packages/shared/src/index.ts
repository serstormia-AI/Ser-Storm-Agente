...
export interface AgencyBrain {
  ...
  name: string;
  slug: string;
  content: string; // Business description & positioning
  services: string; // Core services list & value props
  tone: string; // Tone & conversation guidelines
  policies: string; // Business policies & pricing ranges
  handoff_rules: string; // Rules when to pass to human
  business_hours: {
    timezone: string;
